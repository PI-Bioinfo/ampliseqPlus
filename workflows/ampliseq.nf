...
include { LEFSE_PREPARE_INPUT           } from '../modules/local/lefse_prepare_input'
include { LEFSE_RUN                     } from '../modules/local/lefse_run'
...
workflow AMPLISEQ {
    ...
    if (!params.skip_lefse) {
            LEFSE_PREPARE_INPUT ( ch_metadata, QIIME2_EXPORT.out.col6_tsv )
            LEFSE_RUN ( LEFSE_PREPARE_INPUT.out.tsv )
        }
